...
def convert_string(s1, s2, index1, index2):
    if index1 == len(s1):
        return len(s2) - index2 # insert all remaining characters from s2
    if index2 == len(s2):
        return len(s1) - index1 # delete all remaining characters from s1
    if s1[index1] == s2[index2]:
        return convert_string(s1, s2, index1 + 1, index2 + 1) # if the characters match, skip both
    else:
        insert = 1 + convert_string(s1, s2, index1+1, index2 ) # insert character from s1 into s2
